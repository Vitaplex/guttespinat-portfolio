...
The "base" HTML-file looks like this, no text, no nothing, just a barebones page being populated with corresponding markdown file content.
```html
<body>
...
The CSS is used in conjunction with rendered markdown to do some special things...  
You might have seen different colored blockquotes around the website.
...
The **DirectoryListing** is a JSON object with all the paths of all the files in the `markdown`-folder in the solution source. The listing is fetched from the  `/api/directorylisting` -endpoint (<a target="_blank" href="/api/directorylisting">See it here</a>)  
